...
The `Scheduled publish` GitHub Actions workflow builds, verifies, and deploys the `main` branch every day at 06:15 Europe/Brussels. GitHub may delay scheduled runs, so this is the trigger time rather than a guaranteed publication time. It can also be run manually. Configure these GitHub Actions repository secrets before enabling it:
